...
async def execute_dynamic_query(sql: str, user_prompt: str, query_name: str, query_description: str) -> str:
    """
    ...
    You MUST provide the `user_prompt` that was asked, a clear `query_name` (e.g. 'find_bloated_tables') 
    ...
    # Guardrail against LLMs bypassing the cache
    if any(q['name'] == query_name for q in SAVED_QUERIES):
        return f"❌ ACTION BLOCKED: A query named '{query_name}' already exists in the cache! You MUST use the 'run_saved_query' tool with this query_name instead of generating it dynamically."
    ...
    try:
        async with get_db_connection() as conn:
            ...
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                # Auto-save successfully executed query to YAML
                try:
                    await save_query(query_name, sql, query_description)
                except Exception as e:
                    logger.warning(f"Failed to auto-save query to YAML: {e}")
                    
                # Auto-save to QueryTemplateStore for semantic matching
                try:
                    db_id = await schema_manager.get_database_id()
                    _, schema_hash = await schema_manager.fetch_schema()
                    query_store.store_query(db_id, schema_hash, user_prompt, sql)
                except Exception as e:
                    logger.warning(f"Failed to save to QueryTemplateStore: {e}")
